            ...
            <div class="col-4">
                <div>
                 ...
                 <div class="info-section color-base-2 mt-5 p-2">
                    ...
                    <p>Phone: (302) 219-3120</p>
                    <p>Address: 412 West 7th Street, STE 912. Clovis, NM 88101</p>
                    <p>Owner: Andress Aloures</p>
                    <p>Email: aloures620@gmail.com</p>
                    <p>DOT: 4297190</p>
                    <p>Period: {{ \Carbon\Carbon::parse($charge->date_start)->format('m/d/Y') }} to {{ \Carbon\Carbon::parse($charge->date_end)->format('m/d/Y') }}</p>
                </div>
            </div>
            <div class="col-4">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Converte valor com separador brasileiro para número (ex: "R$ 1.845,00" → 1845.00)
    function getBrazilianNumber(text) {
        return parseFloat(
            text.replace(/[^\d,]/g, '')  // remove tudo que não for número ou vírgula
                .replace(/\./g, '')      // remove pontos (milhar)
                .replace(',', '.')      // troca vírgula decimal por ponto
        ) || 0;
    }

    // Conversão padrão, usada para os outros valores (ponto é decimal, vírgula é milhar)
    function getNumericValue(text) {
        return parseFloat(text.replace(/[^\d.-]/g, '')) || 0;
    }

    function updateTotal(priceToRemove) {
        const totalEl = document.getElementById('total-price');
        const totalElPrint = document.getElementById('total-price-print');

        // Aqui usamos o tratamento especial
        let currentTotal = getBrazilianNumber(totalEl.textContent);

        let newTotal = currentTotal - priceToRemove;
        if (newTotal < 0) newTotal = 0;

        totalEl.textContent = formatter.format(newTotal);
        totalElPrint.textContent = formatter.format(newTotal);
    }
    ...
    document.querySelectorAll('.btn-remove').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            const confirmDelete = confirm('Tem certeza de que deseja remover esta linha da fatura?');
            if (!confirmDelete) return;

            // Remover linha da visualização
            const rowView = document.getElementById(`row-view-${id}`);
            if (rowView) {
                const priceText = rowView.querySelector('td:nth-child(3)').textContent;
                const price = getNumericValue(priceText); // aqui usamos o padrão
                rowView.remove();
                updateTotal(price);
            }

            // Remover linha da impressão
            const rowPrint = document.getElementById(`row-print-${id}`);
            if (rowPrint) {
                rowPrint.remove();
            }
        });
