...
class AttachementController extends Controller
{
    ...
    {
        $attachments = Attachment::with('user')->paginate(10);
    ...
    {
        $attachments = Attachment::with('user')->where('user_id', $id)->get();
        return view('attachment.index', compact('attachments'));
    }

    public function create2(string $id)
    {
        $user = User::findOrFail($id); // ou ->find($id) e tratar null
        // criamos uma coleção com um único elemento
        $users = collect([$user]);
    ...
    public function store(Request $request)
    {
       ...
       return redirect()
    ->route('attachments.index2', ['id' => $request->user_id])
    ...
    public function destroy(string $id)
    {
        ...
        return redirect()
            ->route('attachments.index2', ['id' => $user_id])
            ->with('success', 'Arquivos enviados com sucesso!');
